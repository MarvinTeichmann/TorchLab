...
from encoding import functions as F
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, num_classes=1000, dilated=True):
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                trans = _Transition(num_input_features=num_features, num_output_features=num_features //
                                    2, stride=strides[i+1], dilation=dilations[i])
    ...
    def get_channel_dict(self):
        ...
        assert(isinstance(bn4, nn.BatchNorm2d))
        bn3 = [m for m in self.layer3.modules()][-2]
        assert(isinstance(bn3, nn.BatchNorm2d))
    ...
    def forward(self, x, verbose=True, return_dict=False):
