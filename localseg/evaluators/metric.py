...
class SegmentationMetric(object):
    ...
    def __init__(self, num_classes, name_list=None):
        ...
        self.max_examples = 20

        if self.name_list is None:
            self.name_list = list(range(num_classes))
    ...
    def compute_miou(self, ignore_first=False, remove_nans=True):

        if remove_nans:
            flt = self.tps + self.fns > 0

        ious = self.tps[flt] / (self.tps[flt] + self.fps[flt] + self.fns[flt])
    ...
    def get_pp_names(self, time_unit='s', summary=False):
        if not summary:
            ret_list = copy.copy(self.name_list[:self.max_examples])
    ...
    def get_pp_values(self, ignore_first=True,
        ...
        if summary:
            ...
            ious = self.tps / (self.tps + self.fps + self.fns)
            values = list(ious[:self.max_examples])
            values.append(pp.NEW_TABLE_LINE_MARKER)

        miou = self.compute_miou(ignore_first=ignore_first)
